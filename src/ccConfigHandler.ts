...
export class CCConfigHandler {
  ...
  private loadConfig(): boolean {
    const config = vscode.workspace.getConfiguration('vscode-clearcase');
    if (config) {
      this.m_changeIdents = [];
  ...
  private setChangeConfigDate<T>(config: vscode.WorkspaceConfiguration,
                                 descriptor: string,
                                 configValue: ConfigurationProperty<T>): boolean {

...
async function _activate(context: vscode.ExtensionContext, disposables: vscode.Disposable[]) {
  ...
  const outputChannel: vscode.OutputChannel = vscode.window.createOutputChannel('Clearcase SCM');
  ...
  provider.updateIsView().then((is: boolean) => {
    ...
    vscode.commands.executeCommand('setContext', 'vscode-clearcase:DynView',
                                   provider.ClearCase.ViewType === ViewType.DYNAMIC);
  });
  ...
  const uiInfo = new UIInformation(context, disposables, configHandler,
                                   vscode.window.activeTextEditor, provider.ClearCase);
